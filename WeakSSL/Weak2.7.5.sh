...
for IP in $(cat $pth/$targets); do
    for PORTNUM in ${PORT[*]};do
        ...
        STAT2=$(cat Nmap/nmap_output.gnmap | grep $IP | grep "Ports: $PORTNUM/open" -m 1 -o | cut -c 11-15)
        if [ "$STAT1" == "Up" ];then 
            if [ "$STAT2" == "open" ];then
...
for IP in $(cat $pth/$targets); do
    for PORTNUM in ${PORT[*]};do
        ...
        STAT2=$(cat $pth/Nmap/nmap_output.gnmap | grep $IP | grep "Ports: $PORTNUM/open" -m 1 -o | cut -c 11-15)
        if [ "$STAT1" == "Up" ];then 
            if [ $STAT2" == "open" ];then
        ...
        if [ "$STAT1" == "Up" ];then 
            if [ "$STAT2" == "open" ];then
                for ciphr in ${Ciphers[*]};do
                    ...
                    echo "Address: $IP"
                    echo "Cipher: $Ciphers"
                    bash /tmp/cipherscan/openssl s_client -connect $IP:$PORTNUM -ssl3 -cipher $ciphr | aha -t "OpenSSL Scan" >> WeakSSL/$IP-WeakCiphers.html
                    echo "---------------------------------------------TLSv1---------------------------------------------------------"
                    echo "Address: $IP"
                    echo "Cipher: $Ciphers"
                    bash /tmp/cipherscan/openssl s_client -connect $IP:$PORTNUM -tls1 -cipher $ciphr | aha -t "OpenSSL Scan" >> WeakSSL/$IP-WeakCiphers.html
                    echo "---------------------------------------------TLSv1.1-------------------------------------------------------"
                    echo "Address: $IP"
                    echo "Cipher: $Ciphers"
                    bash /tmp/cipherscan/openssl s_client -connect $IP:$PORTNUM -tls1_1 -cipher $ciphr | aha -t "OpenSSL Scan" >> WeakSSL/$IP-WeakCiphers.html
                    echo "---------------------------------------------TLSv1.2-------------------------------------------------------"
                    echo "Address: $IP"
                    echo "Cipher: $Ciphers"
                    bash /tmp/cipherscan/openssl s_client -connect $IP:$PORTNUM -tls1_2 -cipher $ciphr | aha -t "OpenSSL Scan" >> WeakSSL/$IP-WeakCiphers.html
                    echo "--------------------------------------------------"
                done
            fi
        fi
