...
masscan -iL $pth/targets -p 0-65535 --open-only --banners -oL $workpth/Masscan/masscan_output
...
for IP in $(cat $workpth/Nmap/livehosts); do
	for PORTNUM in ${PORT[*]}; do
		...
		if [ "$STAT1" == "Up" ] && [ "$STAT2" == "open" ] || [ "$STAT3" == "filtered" ];then
			if [ "$PORTNUM" == "443" ]
				python jexboss.py -u https://$IP:443 -results "$workpth/JexBoss/$IP-$PORTNUM-HTTPS-JexBOSS" -out "$workpth/JexBoss/Logs/$IP-$PORTNUM-report_file_scan2.log" | tee -a $workpth/JexBoss/Logs/$IP-$PORTNUM
			fi
